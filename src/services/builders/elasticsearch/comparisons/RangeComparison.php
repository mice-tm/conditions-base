...
class RangeComparison implements ComparisonInterface
{
    ...
    public function buildFilter(ConditionInterface $condition): array
    {
        if (AttributeInterface::GREATER_THAN_COMPARISON === $condition->comparison) {
            $query["range"][$condition->attribute][DefaultComparison::RANGE_PARAMETER_GREATER_THAN]
                = $condition->value;
            return $query;
        }
        if (AttributeInterface::LESS_THAN_COMPARISON === $condition->comparison) {
            $query["range"][$condition->attribute][DefaultComparison::RANGE_PARAMETER_LESS_THAN]
                = $condition->value;
            return $query;
        }
        if (AttributeInterface::GREATER_THAN_OR_EQUAL_TO_COMPARISON === $condition->comparison) {
            $query["range"][$condition->attribute][DefaultComparison::RANGE_PARAMETER_GREATER_THAN_OR_EQUAL_TO]
                = $condition->value;
            return $query;
        }
        if (AttributeInterface::LESS_THAN_OR_EQUAL_TO_COMPARISON === $condition->comparison) {
            $query["range"][$condition->attribute][DefaultComparison::RANGE_PARAMETER_LESS_THAN_OR_EQUAL_TO]
                = $condition->value;
            return $query;
        }
